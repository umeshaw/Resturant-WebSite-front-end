...
function Navbar() {
  ...
  return (
    <nav className='fixed top-4 z-50 flex w-full flex-col items-center'>
        ...
        <div className='hidden space-x-6 lg:flex'>
          {LINKS.map((link, index) => (
            <a
              key={index}
              href={`#${link.targetId}`}
              className={`text-sm text-white ${index !== 0 ? 'border-l-2 border-neutral-300/20 pl-2' : ''} hover:opacity-50`}
      ...
      {isMobileMenuOpen && (
        <div className='w-full backdrop-blur-lg lg:hidden flex flex-col items-center space-y-4 mt-4 '>
          {LINKS.map((link, index) => (
            <a
              key={index}
              href={`#${link.targetId}`}
              className='block p-4 text-sm text-white hover:opacity-50 uppercase tracking-tighter'
              onClick={(e) => handleScroll(e, link.targetId)}
            >
              {link.text}
            </a>
